...
def get_crossover(i, timeframe, sma_fast, sma_medium, sma_slow):
    if i < sma_slow:
        return 0
    sma_slow_now = get_sma(timeframe[:i + 1], sma_slow)
    sma_slow_past = get_sma(timeframe[:i + 1], sma_slow)
    sma_medium_now = get_sma(timeframe[:i], sma_medium)
    sma_medium_past = get_sma(timeframe[:i], sma_medium)
    sma_fast_now = get_sma(timeframe[:i + 1], sma_fast)
    sma_fast_past = get_sma(timeframe[:i], sma_fast)

    verdict_now = 1 if sma_fast_now > sma_medium_now > sma_slow_now or sma_fast_now < sma_medium_now < sma_slow_now else 0
    verdict_past = 1 if sma_fast_past > sma_medium_past > sma_slow_past or sma_fast_now < sma_medium_now < sma_slow_now else 0
    indication = verdict_now - verdict_past
    return indication


def long_condition(i, timeframe, sma_fast, sma_medium, sma_slow):
    crossover = get_crossover(i, timeframe, sma_fast, sma_medium, sma_slow)
    if crossover == 1:
        # print("CROSS UP")
        return True
    return False
...
def short_condition(i, timeframe, sma_fast, sma_medium, sma_slow):
    crossover = get_crossover(i, timeframe, sma_fast, sma_medium, sma_slow)
    if crossover == -1:
        # print("CROSS DOWN")
        return True
...
def test(timeframe, sma_fast, sma_medium, sma_slow, balance, spread):
    status = False
    type = None
    open_price = None
    wins = 0
    ...
    for i in range(len(timeframe)):
        ...
        if status:
            ...
            if type == "up":
                short = short_condition(i, timeframe, sma_fast, sma_medium, sma_slow)
                if short:
            ...
            if type == "down":
                long = long_condition(i, timeframe, sma_fast, sma_medium, sma_slow)
...
if __name__ == "__main__":
    ...
    for ticker in tickers:
        ...
        result = test(minutes, sma_fast, sma_medium, sma_slow, balance, spread)
